...
def qinit(state,f,xin,period,num_mesh,num_period):
    ...
    for i in range(8000):
        for j in range(100):
            r2=(X[i,j]-5.1)**2 + (Y[i,j]-0.5)**2/30
            if r2<0.01:
                perturbation[i,j]=np.exp(-0.04/(0.01-r2))
...
def setup(f=6.0,hn=0.28,num_mesh=800,num_period=10,kernel_language='Python', use_petsc=False, outdir='roll_width_1',
