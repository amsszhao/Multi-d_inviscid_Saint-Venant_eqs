clc
clear
filepath=strcat(pwd,'\roll_width_point36_periodic');
N=5001;
filenamev='roll_width_point36_periodic';
hn=0.28;
f=6;
num_mesh=800;
num_period=10;
...
for i=1:N
    ...
    ax.YLim = [0 0.36];
    ax.CameraPosition =[(X_lower+X_upper)/2 -0.18 5];
    ax.CameraTarget = [(X_lower+X_upper)/2 0.18 1.5];
    ...
    t = annotation('textbox','String','$F=6$, $H_-=0.28$ width is $0.36$ with periodic $y$--boundary condition','Interpreter','latex','EdgeColor','none');
    t.FontSize=20;
    t.Position=[0.345,0.88,0.384,0.07];
    t.Color=[1 1 1];
    axis off
    ax2=subplot(2,1,2);
