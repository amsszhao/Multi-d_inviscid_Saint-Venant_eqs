...
def qinit(state,HL=1.,dam_break_location=25):
    ...
    for i in range(5000):
        for j in range(500):
            r2 =(X[i,j]-10.0)**2 + (Y[i,j]-2.5)**2
            if r2<=0.5:
                perturbation[i,j]=np.exp(-0.2/(0.5-r2))
    state.q[depth     ,:,:] = HL+perturbation
    state.q[x_momentum,:,:] = HL*UL
...
def setup(F=2.25,kernel_language='Python', use_petsc=False, outdir='flat_F_equals_2_point_25',
